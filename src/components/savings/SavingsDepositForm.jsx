...
import { AlertTriangle, Wallet, ChevronDown, MoreHorizontal } from 'lucide-react';
...
export default function SavingsDepositForm({ isOpen, onClose, goal, mode = 'deposit' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation logic for fund source balance if deposit
    if (isDeposit && fundSourceId) {
    ...
    try {
      if (isDeposit) {
        await addSavingsDeposit(goal.id, amount, note.trim(), fundSourceId);
        if (amount >= remaining) {
        ...
        await withdrawSavings(goal.id, amount, note.trim(), fundSourceId);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={isDeposit ? 'Tabung Dana' : 'Tarik Dana'} size="sm">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {fundSources && fundSources.length > 0 && (
          <div>
            <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">
              {isDeposit ? 'Sumber Dana (opsional)' : 'Tujuan Dana (opsional)'}
            </label>
              ...
              >
                {selectedSource ? (
                  ...
                  <>
                    ...
                    <span className="flex-1 text-surface-400">
                      Pilih {isDeposit ? 'sumber' : 'tujuan'} dana...
                    </span>
                  </>
                )}
