          ...
          <div className="space-y-4 mb-6">
            {[{l:'Pemasukan',v:monthlyStats?.income||0,c:'text-emerald-600 dark:text-emerald-400',dot:'bg-emerald-500'},
              ...
              <div key={i.l} className="flex items-center justify-between">
                <div className="flex items-center gap-3"><div className={`w-3 h-3 rounded-full ${i.dot}`}/><span className="text-sm text-surface-600 dark:text-surface-400">{i.l}</span></div>
                <span className={`text-sm font-bold ${i.c}`}>{formatCurrency(i.v)}</span>
              </div>
            ))}
